...
std::unique_ptr<JSONDict> getVariadicJSON(const Constraint &constraint) {
  ...
  json->insertJson(
      "baseType",
      getJSON(Constraint(constraint.getDef()->getValueAsDef("baseType"))));
  return json;
}
...
std::unique_ptr<JSONDict> getTypeDefJSON(const Constraint &constraint) {
  ...
  json->insert(
      "dialect",
      constraint.getDef()->getValueAsDef("dialect")->getValueAsString("name"));
  json->insert("name", constraint.getDef()->getName());
  return json;
}

std::unique_ptr<JSON> getJSON(const Constraint &constraint) {
  if (constraint.getDef()->isSubClassOf("TypeDef")) {
    auto res = getTypeDefJSON(constraint);
    res->insert("kind", "typeDef");
    return res;
  }

  if (constraint.getDef()->isSubClassOf("Variadic")) {
    auto res = getVariadicJSON(constraint);
    res->insert("kind", "variadic");
    return res;
  }

  if (constraint.getDef()->isSubClassOf("Optional")) {
    auto res = getOptionalJSON(constraint);
    res->insert("kind", "optional");
    return res;
  }

  if (constraint.getDef()->isSubClassOf("I")) {
    auto res = JSONDict::get();
    res->insert("kind", "integer");
    res->insert("bitwidth",
                (int)constraint.getDef()->getValueAsInt("bitwidth"));
    return res;
  }
  ...
  auto superclasses = constraint.getDef()->getSuperClasses();
